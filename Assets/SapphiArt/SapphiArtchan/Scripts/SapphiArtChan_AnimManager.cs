...
public class SapphiArtChan_AnimManager : MonoBehaviour {
    ...
    internal string AnimatorState = null;
    internal bool animatorPlaying = false;
    ...
    void Start()
    {
        ...
        //右足のコライダーを取得
        parent = GameObject.Find("Foot_R");
        footCollider = parent.transform.Find("Toe_R").GetComponent<SphereCollider>();
    ...
    void SetAnimation()
    {
        ...
        if ( _SapphiArtChanAnimation == "hit01" || _SapphiArtChanAnimation == "hit02" || _SapphiArtChanAnimation == "hit03" )
        {
            //アニメーション設定時にコライダーを有効化
    ...
    void ReturnToIdle()
    {
        ...
        else if(_SapphiArtChanAnimator.GetCurrentAnimatorStateInfo(0).IsName("idle") )
        {
    ...
    void ReturnToIdle_AnimaState()
    {
        ...
        if( _SapphiArtChanAnimation != "idle")
        {
            ...
            IslastPlaying = animatorPlaying;

            if (_SapphiArtChanAnimator.GetCurrentAnimatorStateInfo(0).IsName("idle")){
                AnimatorState = "idle";
            }
            else { 
                AnimatorState = "not idle";
            }

            //アニメーション再生中？
            animatorPlaying = AnimatorState != "idle";

            //アニメーション開始
            if (animatorPlaying == true)
            {
                SetAllAnimationFlagsToFalse();
                _SapphiArtChanAnimator.SetBool("param_toidle", true);
            }
            ...
            //アニメーションが終了？
            if(IslastPlaying && (!animatorPlaying))
            {
                _SapphiArtChanAnimation = null;
                //アニメーション終了したらコライダーを無効化
                footCollider.enabled = false;
            }
